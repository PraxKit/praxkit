defmodule PraxkitWeb.Admin.ResetPasswordController do
  use PraxkitWeb, :controller

  alias Praxkit.{Admins, Emails, Mailer, Admins.Guardian}

  plug PraxkitWeb.Plugs.RedirectAdmin
  ...
  def create(conn, %{"admin" => %{"email" => email}}) do
    if admin = Admins.get_admin_by_email(email) do
      token = Phoenix.Token.sign(PraxkitWeb.Endpoint, "admin_auth", admin.id)
  ...
  def show(conn, %{"token" => token}) do
    case Phoenix.Token.verify(PraxkitWeb.Endpoint, "admin_auth", token, max_age: 600) do
      {:ok, id} ->
