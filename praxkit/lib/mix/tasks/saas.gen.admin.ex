defmodule Mix.Tasks.Saas.Gen.Admin do
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix saas.gen.admin Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix saas.gen.admin Sales User users --web Sales
  ...
  alias Mix.Tasks.Saas

  @doc false
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise "mix saas.gen.admin can only be run inside an application directory"
    end

    args =
      case Enum.member?(args, "--web") do
        true -> args
        false -> args ++ ["--web", "Admin"]
      end
    ...
    """
    accountscope = if Mix.shell().yes?("Belong to an account?"), do: true, else: false
    ...
    context
    |> copy_new_files(binding, paths, accountscope)
  ...
  defp files_to_be_generated(%Context{schema: schema, context_app: context_app}, accountscope \\ false) do
    web_prefix = Mix.Phoenix.web_path(context_app)
    test_prefix = Mix.Phoenix.web_test_path(context_app)
    web_path = to_string(schema.web_path)
    live_subdir = "#{schema.singular}_live"
    scope_prefix = if accountscope, do: "accountscoped_", else: ""

    [
      {:eex, "#{scope_prefix}show.ex", Path.join([web_prefix, "live", web_path, live_subdir, "show.ex"])},
      {:eex, "#{scope_prefix}index.ex", Path.join([web_prefix, "live", web_path, live_subdir, "index.ex"])},
      {:eex, "#{scope_prefix}form_component.ex", Path.join([web_prefix, "live", web_path, live_subdir, "form_component.ex"])},
      {:eex, "form_component.html.heex", Path.join([web_prefix, "live", web_path, live_subdir, "form_component.html.heex"])},
      {:eex, "#{scope_prefix}index.html.heex", Path.join([web_prefix, "live", web_path, live_subdir, "index.html.heex"])},
      {:eex, "show.html.heex", Path.join([web_prefix, "live", web_path, live_subdir, "show.html.heex"])},
      {:eex, "#{scope_prefix}live_test.exs", Path.join([test_prefix, "live", web_path, "#{schema.singular}_live_test.exs"])},
  ...
  defp copy_new_files(%Context{} = context, binding, paths, accountscope) do
    files = files_to_be_generated(context, accountscope)
    Mix.Phoenix.copy_from(paths, "priv/templates/saas.gen.admin", binding, files)
    if context.generate?, do: Saas.Gen.Context.copy_new_files(context, paths, binding, accountscope)
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: ctx_app} = context) do
    ...
    if context.generate?, do: Saas.Gen.Context.print_shell_instructions(context)
  end
  ...
  defp inputs(%Schema{} = schema) do
    Enum.map(schema.attrs, fn
      ...
      {key, :float} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, class: "input input-bordered", step: "any" %>), error(key)}
      {key, :decimal} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, class: "input input-bordered", step: "any" %>), error(key)}
      {key, :boolean} ->
        {checkbox_label(key), ~s(<%= checkbox f, #{inspect(key)}, class: "checkbox" %>), error(key)}
      {key, :text} ->
        {label(key), ~s(<%= textarea f, #{inspect(key)}, class: "textarea textarea-bordered" %>), error(key)}
      {key, :date} ->
      ...
      {key, _}  ->
        {label(key), ~s(<%= text_input f, #{inspect(key)}, class: "input input-bordered" %>), error(key)}
    end)
  end

  defp label(key) do
    ~s(<%= label f, #{inspect(key)}, class: "label label-text" %>)
  end

  defp checkbox_label(key) do
    ~s(<%= label f, #{inspect(key)}, class: "cursor-pointer label label-text ml-4" %>)
  end
